...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await api.post('/auth/register', {
        did:             form.did,
        public_key_hex:  form.publicKeyHex,
        wallet_address:  form.walletAddress,
        private_key_hex: form.privateKeyHex,
      })
      // Show warning if blockchain was skipped
  ...
  return (
    <div style={{
      minHeight: '100vh', position: 'relative', background: '#060a12',
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 420 }}>
        ...
        <div className="card" style={{ borderRadius: 22, padding: 30 }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {error && (
              <div className="alert alert-error" style={{ marginBottom: 13 }}>
                {error}
              </div>
            )}
      ...
      {qrData && (
        <QRModal
          qrBase64={qrData.qr}
          totpUri={qrData.uri}
          onClose={() => { setQrData(null); navigate('/login') }}
        />
      )}
    </div>
  )
